...
const TaskDetails = () => {
  ...
  const handleAddComment = async () => {
    ...
    try {
      toast.loading("Adding comment...", { id: "comment" });
      const res = await API.post(`/tasks/${taskId}/comments`, { content: newComment });
      setComments((prev) => [...prev, res.data.comment]);
      setNewComment("");
      toast.success("Comment added.", { id: "comment" });
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message, { id: "comment" });
    }
  };
                ...
                {comments.map((comment) => (
                  <div
                    ...
                    className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${
                      comment.user?._id === user?._id ? "ml-auto" : "mr-auto"
                    }`}
                  >
        ...
        <div className="p-5 rounded-md bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800">
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              <span className="px-2 py-0.5 rounded bg-zinc-200 dark:bg-zinc-700 text-xs">{task.status}</span>
