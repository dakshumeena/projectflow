...
const ProjectTasks = ({ tasks, onTaskUpdated, onTasksDeleted, canManageTasks = true }) => {
  ...
  // keep localTasks in sync when parent re-fetches
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      ...
      setLocalTasks((prev) => prev.map((t) => (t._id === taskId ? data.task : t)));
      if (onTaskUpdated) onTaskUpdated(data.task);
      toast.success("Status updated", { id: "status" });
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message, { id: "status" });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (!confirm) return;
    try {
      toast.loading("Deleting tasks...", { id: "delete" });
      await Promise.all(selectedTasks.map((id) => deleteTask(id)));
      setLocalTasks((prev) => prev.filter((t) => !selectedTasks.includes(t._id)));
      if (onTasksDeleted) onTasksDeleted(selectedTasks);
      setSelectedTasks([]);
      toast.success("Tasks deleted", { id: "delete" });
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message, { id: "delete" });
    }
  };
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        {["status", "type", "priority", "assignee"].map((name) => (
          <select
            ...
            className="border not-dark:bg-white border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900 outline-none px-3 py-1 rounded text-sm text-zinc-900 dark:text-zinc-200"
          >
              ...
              <tr>
                <th className="pl-2 pr-1">
                  <input
                    ...
                    checked={selectedTasks.length === localTasks.length && localTasks.length > 0}
                    onChange={() =>
                      selectedTasks.length === localTasks.length
                        ? setSelectedTasks([])
                        : setSelectedTasks(localTasks.map((t) => t._id))
                    }
                  />
                </th>
              ...
              {filteredTasks.length > 0 ? (
                filteredTasks.map((task) => {
                  ...
                  return (
                    <tr
                      key={task._id}
                      onClick={() => navigate(`/taskDetails?projectId=${task.project}&taskId=${task._id}`)}
